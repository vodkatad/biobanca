...
subset <- read.table(subset_f, sep="\t", header=TRUE)
# build .x and .y
wanted <- c(paste0(subset[,1], '.x'), paste0(subset[,1], '.y'))
mergemut_sel <- mergemut[, colnames(mergemut) %in% wanted]

print(ncol(mergemut)/2)
print(ncol(mergemut_sel)/2)
print(nrow(subset))
print(setdiff(wanted, colnames(mergemut_sel)))

mergemut <- mergemut_sel
