...
## parallelo perchè proviamo a togliere i not established 
## perpendicolare per aggiungere bcat e perchè ho finito la fantasia
...
#### aggiungo bcat
bcat_f <- "/home/mferri/BCAT_mutation.tsv"
bcat <- read.table(bcat_f, quote = "", sep = "\t", header = TRUE, stringsAsFactors = FALSE)

res <- merge(res, bcat, by="CASE", all.x=TRUE)
## remove hcc
...
res <- res[c("CASE", "AGE.AT.COLLECTION", "SEX", "SITE.OF.PRIMARY", "STAGE", "THERAPY.BEFORE..Y.N.",
             "KRAS", "BRAF", "NRAS", "CTNNB1", "MSI_MSS", "buoni")]
...
res$CTNNB1 <- as.factor(res$CTNNB1)
...
#res <- res %>% filter(!buoni == "Not established")
...
fit.full <- glm(buoni ~ SITE.OF.PRIMARY + STAGE + THERAPY.BEFORE..Y.N. + KRAS + NRAS + BRAF + CTNNB1 + SEX + AGE.AT.COLLECTION, data = res, family = binomial())
fit.full <- glm(buoni ~ CTNNB1, data = res, family = binomial())
