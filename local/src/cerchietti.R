...
data <- read.table(file.path(dir, 'clinical_data_circos2.tsv'), sep="\t", header=TRUE, stringsAsFactors = FALSE, row.names = 1)
cols <- read.table(file.path(dir, 'clinical_data_circos_cols_emendati.tsv'), sep="\t", header=TRUE, stringsAsFactors = FALSE)
cols[is.na(cols)] <- "NA"


data[is.na(data$AGE.AT.COLLECTION),'AGE.AT.COLLECTION'] <- 0

outcircles <- file.path(dir, 'circle.pdf')
outlegend <- file.path(dir, 'circlelegend.pdf')
...
for (i in seq(1, length(order_cols))) {
  ...
  if (unique(names(col_i))[1] == "numeric") {
    ...
    if (col_i[[1]][2] == "minus") { # we may want to reverse the palette in some cases
      brewer <- rev(brewer)
    } 
