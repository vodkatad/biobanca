...
def next_bed_entry(bedcn):
    ...
    if line != '':
        line.rstrip('\n')
        entry = line.split('\t')
        return([entry[0], int(entry[1]), int(entry[2]), [float(x) for x in entry[3:]]])
    return(None)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--bedcn', '-c', dest='bedcn', action='store', help='a pseudo bed file with cn in the fields from 4th onward. Has to be sorted!', required=True)
    ...
    with open(args.bedcn, 'r') as bedcn:
        while not done:
            ...
            overlap = []
            overlaplen = []
            # get bed entries overlapping with the bin
            ...
            if len(overlap) != 0:
                ...
                for i in range(0, len(overlap)):
                    ...
                    for j in range(0, len(cns)):
                        cn[j] += cns[j] * overlaplen[i]
                    ovlen += overlaplen[i]
                cn = [x / ovlen for x in cn]
            cnp = "\t".join([str(x) for x in cn])
            print('{}\t{}\t{}\t{}'.format(bin[0],bin[1],bin[2], cnp))
