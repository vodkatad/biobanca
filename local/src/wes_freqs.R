...
g <- read.table(genes, header=TRUE, sep="\t", row.names=1)

all_genes <- unique(g$genes)
...
gpdo <- g[match(rownames(pdo), rownames(g)),, drop=F] # we keep info for muts available in our AF tables
gpdx <- g[match(rownames(pdx), rownames(g)),, drop=F]
