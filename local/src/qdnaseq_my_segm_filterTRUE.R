...
if (is.null(opt$rdata) | !is.null(opt$help) | is.null(opt$outseg) | is.null(opt$good)) {
...
my_segm_nocall <- function (obj, outfile, filter_samples) 
{
  ...
  for (i in 1:ncol(segments)) {
    ...
    smodel <- substr(fnames[i], 0, 7)
    if (smodel %in% filter_samples$smodel) {
      if (i == 1) {
...
my_segm_nocall(copyNumbersSegmented, outsegf, keep)
